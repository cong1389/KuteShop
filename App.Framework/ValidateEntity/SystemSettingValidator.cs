...
namespace App.Framework.ValidateEntity
{
	public class SystemSettingValidator : AbstractValidator<SystemSettingViewModel>
	{
		public SystemSettingValidator()
		{
            ...
            base.RuleFor<HttpPostedFileBase>((SystemSettingViewModel x) => x.LogoFooter).Must<SystemSettingViewModel, HttpPostedFileBase>(new Func<HttpPostedFileBase, bool>(SystemSettingValidator.IsValidFileType)).WithMessage<SystemSettingViewModel, HttpPostedFileBase>("Hình ảnh Logo không đúng định dạng");

            base.RuleFor<HttpPostedFileBase>((SystemSettingViewModel x) => x.Favicon).Must<SystemSettingViewModel, HttpPostedFileBase>(new Func<HttpPostedFileBase, bool>(SystemSettingValidator.IsValidFileType)).WithMessage<SystemSettingViewModel, HttpPostedFileBase>("Hình ảnh Favicon không đúng định dạng");
		}
