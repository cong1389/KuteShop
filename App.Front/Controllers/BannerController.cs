...
namespace App.Front.Controllers
{
	public class BannerController : FrontBaseController
	{
		...
		{
			IEnumerable<Banner> banners = _bannerService.FindBy(x => !x.MenuId.HasValue && x.Status == 1 && x.PageBanner.Position == 10 && (!x.FromDate.HasValue || DbFunctions.DiffHours(x.ToDate.Value, DateTimeOffset.UtcNow.Offset) >= (int?)0) && (!x.ToDate.HasValue || DbFunctions.DiffHours(x.ToDate.Value, DateTimeOffset.UtcNow.Offset) <= (int?)0), false);
